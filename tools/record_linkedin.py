...
(() => {
  ...
  end.innerHTML = '<p class="eye">Burling · v2</p><h2>The dump, <em>mapped</em>.</h2><p>Synthetic sample files on a locked workplace series. Ring size is count — not importance. A human still decides what to delete.</p><dl><div><dt>Files</dt><dd>68</dd></div><div><dt>Series</dt><dd>13</dd></div><div><dt>Live dump</dt><dd>None</dd></div></dl>';
...
def record() -> Path:
    ...
    with sync_playwright() as p:
        ...
        caption(page, "Burling", "Watch the map work.")
        page.wait_for_timeout(900)
        shot(page, "00-title")
        page.wait_for_timeout(1800)
        page.evaluate("() => window.__demo.end(false)")

        caption(page, "Sample dump", "Synthetic files. One workplace series each.")
        page.wait_for_timeout(1800)
        shot(page, "01-hero")
        ...
        caption(page, "Drill in", "Click a series. The files open.")
        clicked = click_label(page, "finance")
        if not clicked:
            click_label(page, "operations")
        page.wait_for_timeout(700)
        shot(page, "03-series-files")
        page.wait_for_timeout(1800)

        caption(page, "Handoff", "Each slice carries the file the successor needs.")
        clicked = click_label(page, "invoice acme") or click_label(page, "q3 budget")
        if not clicked:
            click_label(page, "invoice")
        ...
        caption(page, "Personal", "Life leftovers stay in their own lane.")
        click_label(page, "personal")
        page.wait_for_timeout(600)
        shot(page, "06-personal")
        page.wait_for_timeout(1800)

        caption(page, "Burling", "A successor inherits a map, not a folder.")
        page.evaluate("() => window.__demo.end(true)")
        page.wait_for_timeout(800)
