"""Tree-walk clerk: locked main, then reuse / invent / combine at each child.
...
2. Given the children already in that folder — reuse one, invent the
   first, or combine siblings that are the same kind of thing?
3. Same question one level down (detail).

A combine verb while filing places *this* letter in the proposed
folder. It does not move siblings. Rehome is a later supervisor pass
(``maintain_plan``). Unmapped only when the body has no substance, and
...
Decide the NEXT folder for THIS document. Look at the children that
already exist. Do not rename the parent.

Output ONLY a valid JSON object:
{
  "reasoning": "1. Existing children are A, B. 2. This file belongs with X.",
  "action": "reuse" | "invent" | "combine" | "empty",
  "name": "kebab-id",
  "merge": ["existing-id", "existing-id"],
  "into": "kebab-id"
}
...
- invent: name a new child. Use when the folder is empty, or when no
  existing child fits and they should not be merged.
- combine: two or more existing children are the same kind of thing as
  this file. merge = those ids (copied exactly). into = the broader
  folder. Files already in merge WILL MOVE into that folder.
...
- Prefer combine over inventing a near-duplicate (windows + macos →
  operating-systems, not a third OS name).
...
class ChildChoice:
    ...
    action: str  # reuse | invent | combine | empty
    name: str
    merge: tuple[str, ...] = ()
...
class WalkState:
    ...
    Best practice: one home per file, always. Combine mutates homes in
    place so the tree a later file sees is the tree a stranger would walk.
    """
    ...
    def place(
        ...
        *,
        ...
        if detail.action == "combine" and detail.merge and len(path) >= 2:
            self.rehome(path, list(detail.merge), detail.name)
...
def walk_one(
    ...
    *,
    ...
    # Combine is a later supervisor pass (maintain_plan). Filing this
    # letter must not rewrite siblings the clerk already walked.
    if sub.action == "combine":
        sub = ChildChoice("invent", sub.name, ())
    ...
    if len(path) >= 2:
        ...
        if detail.action == "combine":
            detail = ChildChoice("invent", detail.name, ())
